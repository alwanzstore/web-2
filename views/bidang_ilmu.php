    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="example1" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        $no = 1;
                        foreach ($bidang_ilmu->index() as $ok): ?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td><?= $ok['id'] ?></td>
                                <td><?= $ok['nama'] ?></td>
                                                ...
                                                <form method="post">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <textarea name="deskripsi" class="form-control" required><?= $ok['deskripsi'] ?></textarea>
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer justify-content-between">
                                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tipe'])) {
                ...
                switch ($_POST['tipe']) {
                    case "simpan":
                        $bidang_ilmu->create($data);
                        echo '<script>alert("Jenis kegiatan berhasil ditambahkan"); location.href="?url=bidang_ilmu";</script>';
                        break;
                    case "edit":
                        $bidang_ilmu->update($_POST['id'], $data);
                        echo '<script>alert("Jenis kegiatan berhasil diedit"); location.href="?url=bidang_ilmu";</script>';
                        break;
                    case "delete":
                        $bidang_ilmu->delete($_POST['id']);
                        echo '<script>alert("Jenis kegiatan berhasil dihapus"); location.href="?url=bidang_ilmu";</script>';
                        break;
                }
            }
            ?>
